...
## 5. ERC-4337 Agent 钱包方案调研

### 5.1 Smart Account 方案全景

| 方案 | 类型 | ERC-7579 | Session Key | Passkey | EntryPoint | Agent 适合度 | Morph 上 |
|------|------|:--------:|:-----------:|:-------:|:----------:|:-----------:|:-------:|
| **ZeroDev Kernel** | 模块化 SA | ✅ | ✅ (多种 Policy) | ✅ | v0.7 | ⭐⭐⭐ 最佳 | ❌ |
| **Coinbase SmartWallet** | 自研 SA | ❌ | ❌ | ✅ | v0.6 | ⭐ 需要 Passkey | ✅ |
| **Biconomy SmartAccountV2** | 模块化 SA | ❌ | ✅ (Merkle Tree) | ❌ | v0.6 | ⭐⭐ 但服务下线 | ✅ |
| **Biconomy Nexus** | ERC-7579 SA | ✅ | ✅ (Smart Sessions) | ✅ | v0.7 | ⭐⭐⭐ | ❌ |
| **Alchemy LightAccount** | 轻量 SA | ❌ | ❌ | ❌ | v0.6/v0.7 | ⭐ 无 session key | ❌ |
| **Safe + 7579 Module** | 多签 SA | ✅ (via 7579) | ✅ (Rhinestone) | ✅ | v0.6/v0.7 | ⭐⭐ | 部分 |
| **SimpleAccount** | 官方参考 | ❌ | ❌ | ❌ | v0.6/v0.7 | ⭐ 最简单 | ❌ |

> 6M+ 账户用 Kernel，7.3M 用 LightAccount，34k 用 Safe 7579

### 5.2 ZeroDev Kernel — Agent 场景最佳

```
架构: ERC-7579 模块化 Smart Account
EntryPoint: v0.7.0
合约: Kernel.sol + ValidationManager.sol

核心设计:
  ├── rootValidator — 默认验证器（通常是 ECDSA）
  ├── IValidator — 可插拔验证模块接口
  ├── IPolicy — 权限策略接口
  ├── IHook — 交易前后拦截器
  ├── IExecutor — 执行模块
  └── ISigner — 签名器接口

Session Key 实现（Permission 系统）:
  6 种内置 Policy:
    ├── Sudo Policy     — 无限制（全权限）
    ├── Call Policy      — 限制可调用的合约 + 函数 + 参数
    ├── Gas Policy       — 限制 gas 花费
    ├── Signature Policy  — 限制可签名的消息类型
    ├── Rate Limit Policy — 限制调用频率
    ├── Timestamp Policy  — 限制时间范围
    └── 自定义 Policy    — 开发者自己写

  Agent 使用方式:
    1. Owner 创建 session key + 绑定 Policy 组合
    2. Agent 用 session key 签名
    3. Kernel 验证: rootValidator → Policy 检查 → 执行
    → 支持多个 Policy 组合（AND 逻辑）

  vs Biconomy 的 Session Key:
    Biconomy: 单一 ABI SVM 做所有检查
    ZeroDev: 多个 Policy 模块组合，更灵活
    Biconomy: v0.6 EntryPoint
    ZeroDev: v0.7 EntryPoint

Morph 部署状态: ❌ 未部署
```

### 5.3 Alchemy LightAccount — 最轻量

```
架构: 精简版 SimpleAccount
EntryPoint: v0.6 + v0.7 都支持
合约: LightAccount.sol（最精简的 ERC-4337 实现）
...
```
架构: Safe 多签钱包 + ERC-7579 适配层（safe7579）
EntryPoint: v0.6 + v0.7
合约: Safe Proxy Factory 在 Morph 上有

Session Key:
  通过 Rhinestone 的 session key module（ERC-7579 模块）
  → 安装到 Safe 上
  → 跟 ZeroDev 类似的 Policy 系统

特点:
  ✅ 最多审计（多年历史）
  ✅ 多签（机构级安全）
  ✅ 通过 7579 支持 session key
  ❌ 架构重（Safe + 7579 适配层 + Module）
  ❌ Gas 开销大

Morph 部署状态:
  Safe Proxy Factory: ✅ 已部署
  Safe 7579 Module: ❌ 未部署
```

### 5.5 Kite — Agent 支付专用链

```
架构: EVM 兼容 L1，专为 Agent 支付设计
EntryPoint: ERC-4337 兼容
链: Chain ID 2366 (主网) / 2368 (测试网)

独特设计:
  三层身份系统（BIP-32 派生）:
    User Identity → Agent Identity → Session Identity
    每层用 DID 标识: did:kite:alice.eth/chatgpt/portfolio-manager-v1

  三层授权签名链:
    Standing Intent（用户签名，定义上限）
      → Delegation Token（Agent 签名，授权 Session）
        → Session Signature（Session 签名，执行交易）
  ...
  可编程托管（Escrow）:
    支付意图 → 资金托管 → 按结果释放
    支持: authorize / capture / void / refund

  SLA 智能合约:
    服务承诺变链上合约
    宕机自动退款，性能不达标自动 slashing

  花费控制定理:
    最大可损失 ≤ Σ(单笔上限 × 每日上限 × 天数)
    数学证明的损失上界

协议兼容:
  ERC-4337 + x402 + A2A (Google) + MCP (Anthropic) + OAuth 2.1
```

### 5.6 结论：Morph Agent Wallet 推荐方案

```
方案 A: 基于 ZeroDev Kernel（推荐，Agent 场景最佳）
  部署 Kernel + ECDSA Validator + Session Key Policy 到 Morph
  + 自建 Bundler（支持 v0.7）
  + 部署 Paymaster
  优点: ERC-7579 模块化，6 种 Policy，最灵活
  缺点: 需要部署较多合约，用 v0.7 EntryPoint

方案 B: 基于 CoinbaseSmartWallet + 自研 Session Key
  复用已有的 CoinbaseSmartWallet（已在 Morph 上）
  自研 Session Key 模块（CoinbaseSmartWallet 不支持 module，需要 wrapper）
  + 自建 Bundler（v0.6）
  优点: 合约已有，Passkey 支持
  缺点: CoinbaseSmartWallet 不可扩展，Session Key 要从头写

方案 C: 参考 Kite 的设计自研
  自研 Smart Account + 三层身份 + Session Key + 微支付通道
  + 自建 Bundler
  优点: 最灵活，可以做差异化
  缺点: 工作量最大（2-3 个月）

方案 D: 部署 SimpleAccount + 自研 Session Key Module
  部署官方 SimpleAccountFactory
  自研 Session Key 合约（参考 Biconomy 的 merkle tree 方案）
  + 自建 Bundler（v0.6）
  优点: 最简单的起步
  缺点: SimpleAccount 功能少，Session Key 要自己写
```
...
├── zerodev/                     ZeroDev Kernel (Agent 最佳方案)
│   ├── kernel/              4 .sol   Kernel v3 核心
│   └── session-key/         6 .sol   ECDSA Validator
├── alchemy/                     Alchemy LightAccount
│   └── light-account/      28 .sol
