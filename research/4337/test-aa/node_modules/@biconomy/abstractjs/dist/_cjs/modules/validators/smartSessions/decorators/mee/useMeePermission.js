...
const useMeePermission = async (meeClient_, parameters) => {
    ...
    const isEnableAndUseSessionDetailExists = sessionDetailsArray.some((sessionDetailsInfo) => sessionDetailsInfo.mode === constants_1.SmartSessionMode.UNSAFE_ENABLE);
    if (!isEnableAndUseSessionDetailExists && mode_ === "ENABLE_AND_USE") {
        throw new Error("ENABLE_AND_USE mode cannot be used with given session details, instead try USE mode directly");
    }
